...
@onready var debug_bullet = preload("res://vendor/Player_Controller/Spawnable_Objects/hit_debug.tscn")
...
func reload() -> void:
	if current_weapon_slot.current_ammo == current_weapon_slot.weapon.magazine:
		...
		if current_weapon_slot.reserve_ammo != 0:
			...
			Sfx.reload_click(global_position)
		else:
